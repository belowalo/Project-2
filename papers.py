...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    with open(DATA_FILE, 'r') as data:
        ...
        for row in file:
            ...
            # Determine the key to use for nesting
            key = year if by_year else categories[0]

            # Construct the nested dictionary
            esteemed_dictionary = result.setdefault(key, {})

            # Traverse nested dictionaries for each category
            for category in categories if by_year else categories[1:]:
                esteemed_dictionary = esteemed_dictionary.setdefault(category, {})

            # Add paper data to the innermost dictionary
            esteemed_dictionary[name] = {
                'authors': authors,
                'name': name,
                'doi': doi,
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    def build_tree_helper(node):
        ...
        for name, child_node in node.items():
            if 'authors' in child_node.keys():
                trees.append(PaperTree(name, [], child_node['authors'],
                                       child_node['doi'], child_node['citations']))
            else:
                subtree = build_tree_helper(child_node)
                trees.append(PaperTree(name, subtree))
        return trees

    trees = build_tree_helper(nested_dict)
    return trees
